...
// [[Rcpp::export]]
List MCMC_noisy(arma::vec y, arma::mat X, arma::mat A_tilde,arma::mat A_star, int n_iter,double mu,double nu, double a_sigma,double b_sigma,double a_xi,double b_xi,double a_q, double b_q, 
                ...
                int r, int r_star, double uj1,arma::vec u0_mean_prior,arma::vec u1_mean_prior, arma::mat Sigma0,arma::mat Sigma1, int k, int burnin) {
 ...
 arma::vec SIGMA2(n_iter, arma::fill::ones);
  ...
  for(int i=0;i<n_iter; ++i) {
     ...
     // update beta
      ...
      //beta = arma::mvnrnd(B_inv*xy,sigma2*B_inv);
      BETA.col(i) = beta;
   ...
   // update tau2_inv_u00
    ...
    if((i+1)%k==0){
      ...
      if(i>=burnin) {
        TP+= A0_sum1;
        FPrate+= (A_tilde_sum-A0_sum1)/(p*(p-1.0)/2.0-a_sum);
        if(a_sum!=0.0) FNrate+= (a_sum-A0_sum1)/a_sum;
        Accu += p*(p-1.0)/2.0-(A_tilde_sum-A0_sum1)-(a_sum-A0_sum1);
        Alpha0Est+=u00;
        Alpha1Est+=u10;
        A_total+=a_sum;
      }
    ...
    TP/=(n_iter-burnin);
    FPrate/= (n_iter-burnin);
    FNrate/= (n_iter-burnin);
    Accu/= (n_iter-burnin)*(p*(p-1.0)/2.0);
    Alpha0Est/=(n_iter-burnin);
    Alpha1Est/=(n_iter-burnin);
    A_total/=(n_iter-burnin);

    
  return List::create(Named("BETA")=BETA.t(),_["SIGMA2"]=SIGMA2,_["THETA"]=THETA.t(),_["TP"]=TP,_["FPrate"]=FPrate,_["FNrate"]=FNrate,_["Accu"]=Accu,
  _["Alpha0Est"]=Alpha0Est,_["Alpha1Est"]=Alpha1Est,
   _["AP"]=A_total);
}
